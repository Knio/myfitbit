...
function make_chart(selector, outer_width, outer_height, day_start) {
    ...
    var sx = d3.scaleTime()
        .domain([new Date(2017, 3, 1), new Date(2018, 0, 1)])
        .range([0, width])
    var sy = d3.scaleLinear()
        .domain([day_start, day_start + 24])
        .range([0, height]);

    var day_width = sx(new Date(2017, 3, 2));
...
function sleep_chart(sleep) {
    ...
    bars.append('rect')
        ...
        .attr('height', function(d) { return chart.sy((d.seconds / 3600) + chart.day_start); })
        .attr('width', chart.day_width)
        .attr('fill', function(d) { return SLEEP_COLORS[d.level]; });

}

function heartrate_chart(heartrate) {
...
sleep_chart(data.sleep);
heartrate_chart(data.heartrate);
